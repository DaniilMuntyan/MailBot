...
class EmailHandler:
    ...
    def __init__(self, login, password):
        ...
        all_id = data[0].split()
        result, data = self.mail.fetch(all_id[-5], "(RFC822)")
        raw_email = data[0][1]
    ...
    def get_to(self):
        my_mail = self.my_mail
        ans = ""
        decode1 = decode_header(my_mail['To'])
        #print(decode1)
        for item in decode1:
            if item[1] is None:
                temp = codecs.getincrementaldecoder('UTF-8')()
                if isinstance(item[0], str):
                    ans += temp.decode(str.encode(item[0])) + " "
                elif isinstance(item[0], bytes):
                    ans += temp.decode(item[0]) + " "
            else:
                temp = codecs.getincrementaldecoder(item[1])()
                ans += str(temp.decode(item[0])) + " "
        #print(ans)
        return ans
            ...
            if file_name is not None:
                ...
                for name in names:
                    ...
                    if decode1[1] is not None:
                        ...
                        file_name += part_name + " "
            if bool(file_name):
                file_path = os.path.join('/home/ubuntu/Dev/Mail bot/File/', file_name)
